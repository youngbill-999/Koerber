...
Feature: Goods Issue Picking MDT MOP: 2 orders in 1 picking trip with parked distribution trip.
  Background:
    Given set global: idSite = "RL1", idClient = "RK1"
 
  Scenario: Picking in ENTGR_01/ENTGR_04-ENT_H1_04 - two orders, one picking trip and parking the distribution trip.
    # Collegeblock
    ...
    ##注意这里是对各自的colleageblock产生pt   
     ...
     Then MDT RESTART "Mdt" is called with: idTerminal = "IPC6491"
      ...
      //picking开始，选择第一项A
       ...
       And save load unit of picking order "JonasPiOrder[0]" as "JonasLuTarget[0]"//这里是第一次出现，这里需要找到该lu做后续的验证
#该服务的相关操作是为商品的分配一个Carrier LU
...
#         |outputparameterDialog.tPrio|500|
       ...
       # park distribution trip
       And MDT USER_INPUT "Mdt" is called with: event = "BF7", menue = null, inputparameter =
         ||
       And MDT USER_INPUT result has: graphName = "PICKING", stateName = "EMPTY", dynamicSubgraph = null and:
         ||
       And MDT USER_INPUT "Mdt" is called with: event = "BEXIT", menue = null, inputparameter =
         ||
       # take distribution trip
       And MDT USER_INPUT "Mdt" is called with: event = "ENTER", menue = "DISTRIB", inputparameter =
         ||
       And MDT USER_INPUT result has: graphName = "DISTRIB", stateName = "SCAN_LU", dynamicSubgraph = null and:
         ||
       And MDT USER_INPUT "Mdt" is called with: event = "BENTER", menue = null, inputparameter =
         |tIdLu|<lu.id:LuPiTrip[0]>|
       And MDT USER_INPUT result has: graphName = "DISTRIB_SUB", stateName = "INFO_DISTRIB_TRIP", dynamicSubgraph = null and:
         |outputparameterDialog.tIdDistib|<distribTrip.id:DiTrip>|
         |outputparameterDialog.tTyp|LUSTAK|
         |outputparameterDialog.tCntOpenPos|2|
#        |outputparameterDialog.tPrio|0|
         |outputparameterDialog.tSIZ||
         |outputparameterDialog.tIdLu|<lu.id:LuPiTrip[0]>|
         |outputparameterDialog.tSOZ||
       And MDT USER_INPUT "Mdt" is called with: event = "BENTER", menue = null, inputparameter =
...
##接着上面的过程，找到LU确认，然后确认location
      ...
      And order "JonasOrder" has 2 order items, saved as collection "JonasOrderItem"
      And order item "JonasOrderItem[0]" has: stat = "30", idArticle = "40067005", qtyActual = 0.0, qtyReserved = 3.0, qtyCanceled = 0.0, typStock = "AV", typLock = "------", wtTarget = ignore, volTarget = ignore, idCre = "AUTO_IT", tsCre = ignore, idUpd = "basis", tsUpd = ignore
      ...
      And order "AndreaOrder" has 2 order items, saved as collection "AndreaOrderItem"
      And order item "AndreaOrderItem[0]" has: stat = "30", idArticle = "40067005", qtyActual = 0.0, qtyReserved = 2.0, qtyCanceled = 0.0, typStock = "AV", typLock = "------", wtTarget = ignore, volTarget = ignore, idCre = "AUTO_IT", tsCre = ignore, idUpd = "basis", tsUpd = ignore
      And order item "AndreaOrderItem[1]" has: stat = "30", idArticle = "40067024", qtyActual = 0.0, qtyReserved = 3.0, qtyCanceled = 0.0, typStock = "AV", typLock = "------", wtTarget = ignore, volTarget = ignore, idCre = "AUTO_IT", tsCre = ignore, idUpd = "basis", tsUpd = ignore
...
#     Then START_PICKING_TRIPS_BY_DOC_EP is called with: idTerminal = "IPC6491" for:
#        | PiTrip[0] |
#      And order "Order" is in status "38"
#      And picking order "PiOrder[0]" has: idPickingTrip = key:PiTrip[0], stat = "20", idCre = "BPS_SCHEDULER", tsCre = now, idUpd = "basis", tsUpd = now
#      And picking order "PiOrder[1]" has: idPickingTrip = key:PiTrip[1], stat = "20", idCre = "BPS_SCHEDULER", tsCre = now, idUpd = "basis", tsUpd = now
#      And picking trip "PiTrip[0]" has: stat = "10", idCre = "basis", tsCre = now, idUpd = "basis", tsUpd = now
#      And picking trip "PiTrip[1]" has: stat = "10", idCre = "basis", tsCre = now, idUpd = "basis", tsUpd = now
